...
def matxRound(M, decPts=4):
    for i, val in enumerate(M):
        for y,v in enumerate(M[i]):
            val[y]=round(v,decPts)
...
# TODO 计算矩阵的转置
def transpose(M):
    (new_column,new_row) = shape(M)
    matrix = [[0 for x in range(new_column)] for y in range(new_row)]
...
def matxMultiply(A, B):
    a_row,a_column = shape(A)
    b_row,b_column = shape(B)
    if a_column == b_row:
        transpose_b = transpose(B)
        list1=[]
        for v in A:
            list2=[]
            for b_v in transpose_b:
                list3=[]
                for i in range(a_column):
                    list3.append(b_v[i]*v[i])
...
def augmentMatrix(A, b):
    row = len(A)
    return [A[z]+b[z] for z in range(row)]
...
def addScaledRow(M, r1, r2, scale):
    to_add_row = [x*scale for x in M[r2]]
    column = shape(M)[1]
    new_row = [to_add_row[x]+M[r1][x] for x in range(column)]
    M[r1] = new_row
...
def gj_Solve(A, b, decPts=4, epsilon = 1.0e-16):
    if len(A) != len(b):
        return None
    new_matrix = augmentMatrix(A,b)
    row,column = shape(new_matrix)
    for i in range(row):
        max = abs(new_matrix[i][i])
        #c is the maximum's index of each column
        c = i       
        for j in range(i,row):
            current_value = abs(new_matrix[j][i])
            if current_value >= max:
                #swap the row of maximum with the row of diagonal element (row i)
                c = j
                max = current_value
        # if the maximum absolute value is nearly zero the matrix is singular 
        if max<epsilon:
            return None
        swapRows(new_matrix,i,c)
        #scale the coefficient of column to 1
        scale = 1.0/new_matrix[i][i]
        scaleRow(new_matrix,i,scale)
        #eliminate every other element of the column
        for z in range(0,row):
            if z == i:
                pass  
            else:                           
                current_value_2 = new_matrix[z][i]
                scale_2 = current_value_2*(-1.0)
                addScaledRow(new_matrix,z,i,scale_2)
    # extract the result and round all elements in result_matrix to certain decimal points
    result_matrix = [[round(x[-1],decPts)] for x in new_matrix]    
...
def calculateMSE(X,Y,m,b):
    ...
    list = ([(val[1]-val[0]*m-b)**2 for val in zip(X,Y)])
...
#       transfer x into matrix
      M_X = [[val,1] for val in X]
      M_X_T = transpose(M_X)
      result_left = matxMultiply(M_X_T,M_X)
      M_Y = [[val] for val in Y]
      result_right = matxMultiply(M_X_T,M_Y) 
